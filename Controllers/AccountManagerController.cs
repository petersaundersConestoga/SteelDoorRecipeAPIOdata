...
namespace SteelDoorRecipeAPIOdata.Controllers
{
    public class AccountManagerController : ODataController
    {
        ...
        public AccountManagerController(rrrdbContext dbContext, ILogger<AccountManagerController> logger)
        {
            _db = dbContext;
            _logger = logger;
        }
        ...
        [EnableQuery]
        public async Task<IActionResult> Put([FromODataUri] int key, Delta<AccountManager> note)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
