...
namespace SteelDoorRecipeAPIOdata.Controllers
{
    public class IngredientListController : ODataController
    {
        ...
        public IngredientListController(rrrdbContext dbContext, ILogger<IngredientListController> logger)
        {
            _db = dbContext;
            _logger = logger;
        }
        ...
        [EnableQuery]
        public async Task<IActionResult> Put([FromODataUri] int key, Delta<IngredientList> note)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
