...
namespace SteelDoorRecipeAPIOdata.Controllers
{
    public class InstructionController : ODataController
    {
        ...
        public InstructionController(rrrdbContext dbContext, ILogger<InstructionController> logger)
        {
            _db = dbContext;
            _logger = logger;
        }
        ...
        [EnableQuery]
        public async Task<IActionResult> Put([FromODataUri] int key, Delta<Instruction> note)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
